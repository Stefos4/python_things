...
class Wordle:
    ...
    def __init__(self,words):
        ...
        self.number = multiprocessing.Value('i')
        self.prec = multiprocessing.Value('i')
    ...
    def __generateLimits(self,number):
        
        limits = []
        size_div = int(len(self.words)/number)

        for i in range(1,number):
            limits.append(size_div*i)

        limits.append(len(self.words))
    ...
    def process_wd(self,tab,affich):
        ...
        for wd_ in tab:
            ...
            if affich:
                ...
                prc_act = self.__pourcentage(self.number.value,len(self.words));

                if prc_act!=self.prec.value:
                    self.prec.value = prc_act
    ...
    def start(self,display_pourc = True,number_process = 3,precision=False):
        
        limits = self.__generateLimits(number_process)
        ...
        #print("Taille infos :",len(self.all_infos))
        
        p1 = multiprocessing.Process(target=self.process_wd,args=(self.words[:limits[0]],display_pourc,));
        processs.append(p1)

        for p_nbr in range(1,number_process):
            p = multiprocessing.Process(target=self.process_wd,args=(self.words[limits[p_nbr-1]:limits[p_nbr]],display_pourc,))
            processs.append(p)
    ...
    def foundMot(self,mot):
        ...
        while len(words_used)<10 and mot!=words_used[-1]:
            ...
            if len(all_words_prob)<10:
                print(all_words_prob)

            wd = Wordle(all_words_prob)
