...
def calculateEnthropie(word,all_wd):
    ...
    for selected in all_wd:
        ...
        if selected!=word:
            ...
            reste = [word_is_in(les_infos,word,wd) for wd in all_wd].count(True)
            
            
            tm_ap = time.time()
            #print(" -Temps Tour inter",tm_ap-tm_av)
...
if __name__== "__main__":
    ...
    for wd_ in words:
        ...
        print("debut tour")
        #print(wd_,":",calculateEnthropie(wd_,words))
        ...
        act_entr = calculateEnthropie(wd_,words);
        
        
        tm_ap = time.time()
            
        print("Temps une tour act :",tm_ap-tm_av)
        # t = threading.Thread(target=calculateEnthropie,args=(wd_,words,lock,));
        # t.start();
        ...
        if act_entr>highest[1]:
            highest = [wd_,act_entr]
